...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    this.mainPool = new Pool({
      ...
      ssl: this.configService.get<string>('DB_HOST', '').includes('render.com') ? { rejectUnauthorized: false } : false,
    });
  ...
  getBranchPool(config: any): any {
    ...
    if (!this.branchPools.has(key)) {
      const pool = new Pool({
        ...
        ssl: config.db_host.includes('render.com') ? { rejectUnauthorized: false } : false, 
      });
  ...
  private async mockQuery(text: string, params: any[]): Promise<any> {
    ...
    if (lowerText.includes('from users where email')) {
        ...
        if (email === 'admin@example.com' || email === 'selcuk.yilmaz@microvise.net') {
            const hash = await bcrypt.hash('123456', 10);
            return { rows: [{ id: 'mock-id', email, password: hash, selected_branch: 0 }] };
        }
        return { rows: [] };
    }
    if (lowerText.includes('insert into users')) {
        return { rows: [{ id: 'mock-new-id', email: params[0], selected_branch: 0 }] };
    }
    if (lowerText.includes('from branches where user_id')) {
        return { rows: [{ name: 'Merkez Şube', kasa_no: 1, db_host: 'mock', db_port: 5432, db_name: 'mock', db_user: 'mock', db_password: 'mock' }] };
    }
    ...
    if (lowerText.includes('from ads_acik') && lowerText.includes('acik_toplam')) {
        // Return summary
        return { rows: [
    ...
    if (lowerText.includes('from ads_odeme') && lowerText.includes('kapali_adisyon')) { // Wait, query structure might differ
    ...
    if (lowerText.includes('from ads_acik') && lowerText.includes('masano = 99999')) {
        return { rows: [
            { adsno: 101, kurye: 'Ahmet Yılmaz', cikis: '10:30:00', donus: null, tarih: new Date().toISOString(), status: 'open' },
            { adsno: 102, kurye: 'Mehmet Demir', cikis: '11:15:00', donus: null, tarih: new Date().toISOString(), status: 'open' }
        ]};
    }

    // 6. Courier Tracking - Closed
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('masano = 99999')) {
        return { rows: [
            { adsno: 90, kurye: 'Ahmet Yılmaz', cikis: '09:00:00', donus: '09:45:00', tarih: new Date().toISOString(), status: 'closed' },
            { adsno: 95, kurye: 'Ali Kaya', cikis: '09:30:00', donus: '10:10:00', tarih: new Date().toISOString(), status: 'closed' }
        ]};
    }
    ...
    if (lowerText.includes('date(kaptar) as tarih')) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            rows.push({ tarih: d.toISOString().split('T')[0], toplam: (Math.random() * 10000 + 5000).toFixed(2) });
        }
        return { rows: rows.reverse() };
    }

    // 8. Payment Types
    if (lowerText.includes('from ads_odeme') && lowerText.includes('payment_name')) {
        return { rows: [
            { payment_name: 'Nakit', total: '8500.00', count: '50', otip: 1 },
            { payment_name: 'Kredi Kartı', total: '12000.00', count: '65', otip: 2 },
            { payment_name: 'Yemek Kartı', total: '3000.00', count: '15', otip: 3 }
        ]};
    }

    // 9. Cancelled Items - Open
    if (lowerText.includes('from ads_acik') && lowerText.includes('sturu in (1,2,4)')) {
        return { rows: [
            { product_name: 'Çorba', quantity: 1, reason: 'Soğuk geldi', date: new Date().toISOString(), order_id: 201, adtur: 0, waiter_name: 'Ayşe', type: 'iade', status: 'open' }
        ]};
    }
    // 10. Cancelled Items - Closed
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('sturu in (1,2,4)')) {
         return { rows: [
            { product_name: 'Kola', quantity: 1, reason: 'Yanlış sipariş', date: new Date().toISOString(), order_id: 150, adtur: 0, waiter_name: 'Fatma', type: 'iptal', status: 'closed' },
            { product_name: 'Çay', quantity: 2, reason: 'İkram', date: new Date().toISOString(), order_id: 155, adtur: 0, waiter_name: 'Ali', type: 'ikram', status: 'closed' }
        ]};
    }

    // 11. Performance - Totals
    if (lowerText.includes('from ads_odeme') && lowerText.includes('total_sales')) {
        return { rows: [{ total_sales: '25000.00', orders_count: '150', total_discount: '450.00' }] };
    }
    // 12. Performance - Duration
    if (lowerText.includes('acilis_saati') && lowerText.includes('kapanis_saati')) {
        return { rows: [
    ...
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('waiter_name')) {
        return { rows: [
    ...
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('group by p.product_name')) {
        return { rows: [
    ...
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('group_name')) {
         return { rows: [
    ...
    if (lowerText.includes('from ads_adisyon') && lowerText.includes('p.product_name as product_name')) {
        return { rows: [
            { product_name: 'Adana Kebap', quantity: 50, total: '12500.00' },
